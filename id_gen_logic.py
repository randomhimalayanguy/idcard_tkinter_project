...
def pic_to_place(img, pic_name, resize_tuple, loc_tuple, output_name, to_crop=False):
    """
    img, pic_name, resize_tuple, loc_tuple, output_name, to_crop=False
    """
    try:
...
def generate(csv_path, batch_year):
    ...
    for i in range(len(df)):
        try:
            rollno = str(df.iloc[i, 3])
            rollno = "0" * (11-len(rollno)) + f"{rollno}"
            mail = str(df.iloc[i, 9])
            name = df.iloc[i, 0]
            year = batch_year
            output_name = f"{name}-{rollno}"
            ...
            # Placing the barcode
            pic_to_place(output_name, "images/amazingBarcode.png", (220, 40), (260, 350), output_name, True)
